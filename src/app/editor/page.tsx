...
export default function EditorPage() {
	...
	useEffect(() => {
		KinetographAPI.getAssets().then((res) => setAssets(res.assets));
		KinetographAPI.getStatus().then((res) => {
		...
		<div className="flex h-screen w-full flex-col bg-[#0c0c0e] text-[#d1d1d1] overflow-hidden selection:bg-amber-500/30">
				...
				<div className="flex items-center gap-4">
					<div className="flex items-center gap-2 group cursor-pointer">
						...
						<span className="text-xs font-bold tracking-[0.2em] uppercase text-zinc-100 group-hover:text-amber-500 transition-colors">
							Kinetograph{" "}
							<span className="font-light text-zinc-500 group-hover:text-amber-700">
								v0.1.0
							</span>
						</span>
					</div>
					...
					<nav className="flex items-center gap-1">
						<button
							...
							className="p-1.5 mr-2 hover:bg-zinc-800 rounded-sm text-zinc-500 hover:text-amber-500 transition-colors"
			...
			<main className="flex-1 flex overflow-hidden">
				<Group orientation="horizontal">
					...
					>
						<div className="flex h-8 items-center gap-2 border-b border-zinc-800 bg-zinc-900/50 px-3">
						...
						<Group orientation="vertical">
							...
							<Panel defaultSize="60%" minSize="30%">
								<div className="flex h-full flex-col overflow-hidden">
									...
									<div className="flex flex-1 items-center justify-center p-4 bg-gradient-to-b from-[#121215] to-[#0c0c0e]">
										{/* Cinematic Viewer Container */}
										<div className="relative aspect-video w-full max-w-5xl overflow-hidden border border-zinc-800 bg-black shadow-[0_0_50px_rgba(0,0,0,0.5)] group">
											{/* Viewer Overlays */}
							...
							<Panel defaultSize="40%" minSize="20%">
								<div className="h-full flex flex-col bg-[#121215] shadow-[inset_0_4px_20px_rgba(0,0,0,0.3)]">
									<div className="flex h-8 items-center justify-between border-b border-zinc-800 bg-zinc-900/80 px-3">
										<div className="flex items-center gap-4">
											<div className="flex items-center gap-2">
									...
									<div className="flex-1 p-4 overflow-y-auto custom-scrollbar">
										{paperEdit ? (
											...
											<div className="flex h-full items-center justify-center text-zinc-600 italic text-[11px] tracking-wider uppercase">
												Waiting for Agent 2 (Scripter) sequence draft...
											</div>
										)}
									</div>
								</div>
