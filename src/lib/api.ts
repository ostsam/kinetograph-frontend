...
export const KinetographAPI = {
  getHealth: () => api.get('health').json<{ status: string; version: string }>(),
  getConfig: () => api.get('config').json<Record<string, string | number>>(),

  getAssets: (type?: string) => 
    api.get('assets', { searchParams: type ? { asset_type: type } : {} }).json<AssetsResponse>(),
  
  uploadAsset: (file: File, type: 'a-roll' | 'b-roll' = 'b-roll') => {
    const formData = new FormData();
    formData.append('file', file);
    return api.post('assets/upload', { 
  ...
  updateClip: (clipId: string, updates: Partial<PaperEditClip>) => 
    api.patch(`paper-edit/clips/${clipId}`, { json: updates }).json<{ status: string; clip: PaperEditClip }>(),
  
  reorderClips: (clipIds: string[]) => 
    api.post('paper-edit/reorder', { json: { clip_ids: clipIds } }).json<{ status: string; total_clips: number }>(),
