    ...
    >
      <div className="flex items-center justify-between px-2 py-1.5 bg-zinc-900/80 border-b border-zinc-800">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Sequence Generator v1.0</span>
        </div>
        <div className="flex items-center gap-1">
           <div className={`h-1 w-1 rounded-full ${isDisabled ? 'bg-zinc-700' : 'bg-emerald-500 animate-pulse'}`} />
           <span className="text-[8px] font-mono text-zinc-600 uppercase">{isDisabled ? 'OFFLINE' : 'ONLINE'}</span>
        </div>
      </div>
      ...
      <div className="p-2 flex flex-col gap-2">
        <textarea
          ...
          placeholder={isDisabled ? "INGEST ASSETS TO WAKE SWARM…" : "ENTER NARRATIVE BRIEF OR EDITING COMMANDS…"}
        ...
        <div className="flex items-center justify-between border-t border-zinc-800/50 pt-2">
          <span className="text-[8px] font-mono text-zinc-600">INPUT_BUFFER: {prompt.length}/2000_BYTES</span>
      ...
      {isDisabled && assets.length > 0 && phase !== Phase.IDLE && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/60 backdrop-blur-[1px]">
           <div className="flex items-center gap-3 px-4 py-2 bg-zinc-900 border border-zinc-800 rounded-sm shadow-2xl">
              ...
              <span className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Agent Processing Sequence…</span>
           </div>
        </div>
      )}
    </form>
