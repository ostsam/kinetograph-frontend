  ...
  return (
    <div
      ...
      className={cn(
        ...
        clip.clip_type === 'synth' && "bg-purple-900/20 border-l-2 border-l-purple-500",
        clip.clip_type === 'a-roll' && "bg-blue-900/20 border-l-2 border-l-blue-500"
      )}
    >
      <div 
        {...attributes} 
      ...
      <div className="flex flex-1 flex-col p-2 overflow-hidden bg-gradient-to-br from-white/[0.02] to-transparent">
        ...
        <div className="mt-auto flex items-center justify-between text-[9px] font-mono tabular text-zinc-500">
          <div className="flex items-center gap-1">
            <span className={isSelected ? "text-amber-500/80" : ""}>{(clip.in_ms / 1000).toFixed(2)}s</span>
      ...
      <div className="absolute left-0 inset-y-5 w-1 bg-black/20 hover:bg-amber-500/50 cursor-ew-resize transition-colors" title="Trim In" />
      <div className="absolute right-0 inset-y-5 w-1 bg-black/20 hover:bg-amber-500/50 cursor-ew-resize transition-colors" title="Trim Out" />
