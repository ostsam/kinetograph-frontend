...
function getTickStepSeconds(pxPerSecond: number) {
	...
	return (
		options.find((seconds) => seconds * pxPerSecond >= MIN_LABEL_SPACING_PX) ??
...
export function TimelineEditor() {
	...
	const handleDragEnd = (event: DragEndEvent) => {
		...
		if (over && active.id !== over.id) {
			...
			reorderClips(newIds);
			KinetographAPI.reorderClips(newIds).catch((err) =>
		...
		(nextZoomLevel: number, focusClientX?: number) => {
			...
			const focusX =
				focusClientX !== undefined
					? focusClientX - viewportRect.left
					: viewport.clientWidth / 2;
			const focusTimeSeconds =
				(viewport.scrollLeft + focusX) / previousPixelsPerSecond;
			...
			requestAnimationFrame(() => {
				...
				const nextScrollLeft = Math.max(
					0,
					focusTimeSeconds * nextPixelsPerSecond - focusX,
				);
	...
	const sequenceDurationMs = useMemo(() => {
		...
		return paperEdit.clips.reduce(
			(total, clip) => total + (clip.out_ms - clip.in_ms),
			0,
		);
	}, [paperEdit]);
	...
	const timelineDurationSeconds = useMemo(() => {
		const durationFromClips =
			effectiveDurationMs / 1000 + TRAILING_PADDING_SECONDS;
		const durationFromViewport =
			trackViewportWidth > 0 ? trackViewportWidth / pxPerSecond : 0;
		return Math.max(
			MIN_TIMELINE_SECONDS,
			durationFromClips,
			durationFromViewport,
		);
	}, [effectiveDurationMs, pxPerSecond, trackViewportWidth]);

	const timelineWidthPx = Math.max(
		trackViewportWidth,
		Math.ceil(timelineDurationSeconds * pxPerSecond),
	);
	...
	return (
		<div className="flex w-full flex-col gap-4">
			...
			>
				...
				<div
					className="p-4 pt-6 relative flex flex-col gap-1 min-w-full"
					style={{ width: `${timelineWidthPx}px` }}
				>
					{paperEdit ? (
						<DndContext
							...
							<div className="flex flex-col gap-4">
								<div className="flex flex-col gap-2">
									...
									<div className="flex gap-[1px] bg-zinc-800 p-0.5 rounded-sm overflow-hidden">
										<button
											className={cn(
												"flex-1 py-1 text-[9px] font-bold rounded-sm transition-all uppercase",
