...
export function AssetDropzone() {
  ...
  const pendingDeleteAsset = assets.find((asset) => asset.id === pendingDeleteAssetId) ?? null;
  ...
  const handleUpload = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      console.info('API upload unavailable. Using local ingest simulation instead.', err);
  ...
  const commitRenameAsset = useCallback((assetId: string) => {
    ...
    <div className="flex flex-col h-full gap-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-zinc-500">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest tabular">{assets.length} ITEMS</span>
        </div>
        <button 
      ...
      >
            ...
            {assets.length === 0 && !isUploading && (
              <div className="flex flex-col items-center justify-center h-40 text-center px-6">
                ...
                <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest leading-relaxed">
                  Bin Empty<br/>
                  <span className="font-normal lowercase opacity-50 italic">Drag media here to begin ingest</span>
                </p>
              </div>
            )}
          </div>
        ...
        {isUploading && (
          <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] flex flex-col items-center justify-center gap-3">
             ...
             <span className="text-[9px] font-black uppercase tracking-widest text-amber-500">Ingesting Payload…</span>
          </div>
        )}
          ...
          <div className="absolute inset-0 z-30 bg-black/70 backdrop-blur-[2px] flex items-center justify-center p-4">
            <div className="w-full max-w-xs rounded-sm border border-zinc-700 bg-[#16161a] p-4 shadow-2xl">
              ...
              <p className="mt-2 text-[10px] text-zinc-400 leading-relaxed break-all">
                This will remove <span className="text-zinc-200">{pendingDeleteAsset.file_name}</span> from the bin.
              </p>
