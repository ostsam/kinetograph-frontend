...
export const useKinetographStore = create<KinetographState>((set, get) => ({
  ...
  setAssets: (assets) => set((state) => ({
    assets,
    selectedAssetId:
      state.selectedAssetId && !assets.some((asset) => asset.id === state.selectedAssetId)
        ? null
  ...
  renameAsset: (assetId, fileName) => set((state) => ({
  ...
  deleteAsset: (assetId) => set((state) => {
    ...
    const remainingAssets = state.assets.filter((asset) => asset.id !== assetId);
    return {
  ...
  updateClip: (clipId, updates) => set((state) => {
    ...
    const totalDurationMs = newClips.reduce((sum, clip) => sum + (clip.out_ms - clip.in_ms), 0);
    return { 
      paperEdit: { ...state.paperEdit, clips: newClips, total_duration_ms: totalDurationMs }
    };
  }),
  ...
  addAssetToTimeline: (assetId) => {
    ...
    const clipType: PaperEditClip['clip_type'] = asset.asset_type === 'a-roll'
      ? 'a-roll'
    ...
    set((current) => {
      ...
      const totalDurationMs = clips.reduce((sum, item) => sum + (item.out_ms - item.in_ms), 0);
